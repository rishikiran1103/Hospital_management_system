...
create table Slot(
    physician text not null,
    "date"  text not null,
    slot1 boolean not null DEFAULT '1',  -- 1 for slot being available 0 for being alloted
    slot2 boolean not null DEFAULT '1',
    slot3 boolean not null DEFAULT '1',
    slot4 boolean not null DEFAULT '1',
    slot5 boolean not null DEFAULT '1',
    free boolean not null DEFAULT '1',  -- 1 for having free slot 0 for not having any
    primary key(physician,"date"),
    foreign key(physician) references Physician(employee_id)
    );
...
BEGIN
  ...
  DELETE FROM test_undergoes T WHERE T.physician=OLD.employee_id;
  DELETE FROM prescribes P WHERE P.physician=OLD.employee_id;
  DELETE FROM  appointment A where A.physician=OLD.employee_id;
  DELETE FROM Admit A  WHERE A.patient in (SELECT P.ssn FROM patient P WHERE P.pcp=OLD.employee_id);
