...
doctor_id = None
SLOT_TIMINGS = ["9:00AM", "10:00AM", "11:00AM", "2:00PM", "3:00PM", "4:00PM"]


def toggle(request):
    employee_id = request.POST.get("doctor_id")
    ...
    curr_date = datetime.strptime(
        formatted_date, '%d-%m-%Y').date()  # current date obj
    if availability == "available":
        curr.execute("DELETE FROM onleave WHERE physician=%s AND date=%s",
                     (employee_id, formatted_date,))
    elif availability == "unavailable":
        # curr.execute("DELETE FROM onleave WHERE physician=%s AND date=", (employee_id,))
        curr.execute("INSERT INTO onleave VALUES(%s,%s) ON CONFLICT DO NOTHING",
                     (employee_id, formatted_date,))
    else:
        availability = None
    curr.execute("SELECT P.name,P.SSN,P.email,A.start FROM patient P,Appointment A WHERE P.SSN=A.patient AND A.physician=%s",
                 (employee_id,))
    patient_list = curr.fetchall()
    new_list = []
    for pt in patient_list:
        if datetime.strptime(pt[3], '%d-%m-%Y').date() < curr_date:
            new_list.append(pt)
    patient_list = new_list
    curr.execute(
        "SELECT p.name,a.start from patient p,appointment a  where p.SSN=a.patient AND a.start=%s", (formatted_date,))
    appointments = curr.fetchall()
    conn.commit()
    curr.close()
    conn.close()
    return render(request, 'doctor.html', {"availability": availability, "doctor_id": employee_id, "patient_list": patient_list, "appointments": appointments})
...
def fdo_task(request):
    ...
    if task == "register":
        ...
        curr.execute("select P.employee_id,P.name from Physician P where P.department=%s and P.employee_id not in (select P1.physician from OnLeave P1 WHERE P1.date=%s);", (department, appointmentdate,))
...
frontDeskOperator.""".format(patien_name=name)
            ...
            for doc in result:
                curr.execute(
                    "SELECT * FROM SLOT S WHERE S.physician=%s AND S.date=%s", (doc[0], appointmentdate,))
                result1 = curr.fetchall()
                if result1 == []:
                    curr.execute(
                        "INSERT INTO SLOT(physician,date) VALUES(%s,%s) ON CONFLICT DO NOTHING", (doc[0], appointmentdate,))
            curr.execute(
                "SELECT P.employee_id,P.name,S.Slot1,S.slot2,S.slot3,S.slot4,S.slot5 FROM Physician P,Slot S WHERE P.employee_id=S.physician AND S.free='1' AND P.department=%s AND S.date=%s", (department, appointmentdate,))
            result = curr.fetchall()
            if result == []:
                # send mail to patient that no doctors available
                subject = "Regarding Appointment Details"
                ...
                Thank you for your understanding and patience in this matter. If you have any questions or concerns, please do not hesitate to reach out to us.
                 ...
                 Best regards,
                 frontDeskOperator.""".format(patient_name=name)
                SendMail(to=email, subject=subject, body=body)
            else:
                physician_selected = random.choice(result)
                print(physician_selected)
                doctor_name = physician_selected[1]
                physician = physician_selected[0]
                slot_id = None
                for i in range(2, 7):
                    if physician_selected[i] == True:
                        slot_id = i-2
                        curr.execute("UPDATE Slot SET Slot{id}='0' WHERE physician=%s AND date=%s".format(
                            id=(i-1)), (physician, appointmentdate,))
                        print(i)
                        break
                if slot_id == 4:
                    curr.execute(
                        "UPDATE Slot  SET free='0' WHERE physician=%s AND date=%s", (physician, appointmentdate,))
                curr.execute("INSERT INTO Patient(SSN,name,PCP,email) VALUES(%s,%s,%s,%s) ON CONFLICT DO NOTHING",
                             (SSN, name, physician, email,))
                print(slot_id)
                appointmentID = physician+str(SSN)+appointmentdate
                curr.execute("INSERT INTO Appointment(appointment_id,patient,physician,start) VALUES(%s,%s,%s,%s) ON CONFLICT DO NOTHING",
                             (appointmentID, SSN, physician, appointmentdate))
                # send mail to patient
                subject = "Regarding Appointment Details"
                body = """Dear {patient_name},

I am writing to confirm that your appointment with Dr.{doctor_name} has been successfully scheduled for {APT_DATE} at {TIME},and is expected to last an hour(approximately).
...
Best regards,
frontDeskOperator. """.format(patient_name=name, doctor_name=doctor_name, APT_DATE=appointmentdate, TIME=SLOT_TIMINGS[slot_id])
            SendMail(to=email, subject=subject, body=body,
                     attachment='static/appointement.jpg')
...
Yours truly,
frontDeskOperator""".format(apt_id=apt_id, pat=pat, patient_name=name, start=start, doctor_name=doctor_name, TIME=SLOT_TIMINGS[slot_id])
        ...
        curr.execute("INSERT INTO Admit(patient,room,date)  VALUES(%s,%s,%s) ON CONFLICT DO NOTHING",
...
def response(request):
    ...
    conn = pg.connect(**conn_params)
    curr = conn.cursor()
    curr.execute("INSERT INTO AppointmentRequests(name,SSN,email,appointmentdate,department,priority) VALUES (%s,%s,%s,%s,%s,%s) ON CONFLICT DO NOTHING",
...
def test_submit(request):
    ...
    conn = pg.connect(**conn_params)
    curr = conn.cursor()
    curr.execute("INSERT INTO test_undergoes(patient,test_id,date,physician,result) VALUES (%s,%s,%s,%s,%s) ON CONFLICT DO NOTHING",
...
def treatment_submit(request):
    ...
    conn = pg.connect(**conn_params)
    curr = conn.cursor()
    curr.execute("INSERT INTO Undergoes(patient,procedure,room,date,physician,result) VALUES (%s,%s,%s,%s,%s,%s) ON CONFLICT DO NOTHING",
...
def doctor(request):
    ...
    curr_date = datetime.strptime(today, '%d-%m-%Y').date()
    # print(today)
    global doctor_id
    ...
    curr.execute("SELECT * FROM onleave O Where O.physician=%s", (doctor_id,))
    if curr.fetchall() != []:
        availability = "unavailable"
    curr.close()
    conn.close()
    return render(request, 'doctor.html', {"availability": availability, "doctor_id": doctor_id, "patient_list": patient_list, "appointments": appointments})
...
def search(request):
    ...
    curr.close()
    conn.close()
    return render(request, 'search.html', {"doctor_id": doctor_id, "patient": patient, "medication": medication, "procedure": procedure})
...
def add_user(request):
    ...
    conn = pg.connect(**conn_params)
    curr = conn.cursor()
    curr.execute("INSERT INTO {}(name,employee_id,password,SSN) VALUES (%s,%s,%s,%s) ON CONFLICT DO NOTHING".format(
...
def doc_add(request):
    ...
    conn = pg.connect(**conn_params)
    curr = conn.cursor()
    curr.execute("INSERT INTO physician (employee_id,name,SSN,password,department,email) VALUES (%s,%s,%s,%s,%s,%s) ON CONFLICT DO NOTHING",
...
def SendMail(to, subject, body, attachment=None):
    ...
    # Compose the email
    # attachments = ['/path/to/attachment.pdf', '/path/to/another_attachment.jpg']

    # Send the email

    yag.send(to=to, subject=subject, contents=body,
            attachments=[attachment, 'static/logo.png'])
